...
The authors use $d-1$ spacial dimension and the sign convention $\eta_{\mu\nu}=diag(-1,1,...,1)$ which implies $\square=\partial^\mu\partial_\mu=-\partial_t^2+\triangle$ and results in a minus sign in the KG equation.
...
\begin{align}
    \mathcal{L}=-\frac{1}{2}\sum_{j=1}^n\left[\eta^{\alpha\beta}\partial_\alpha\phi^j\partial_\beta\phi^j+m^2(\phi^j)^2\right]
\end{align}
the action functional is then
\begin{align}
    S&=\int d^dx\mathcal{L}\\
    &=-\frac{1}{2}\sum_{j=1}^n\int d^dx\left[\eta^{\alpha\beta}\partial_\alpha\phi^j\partial_\beta\phi^j+m^2(\phi^j)^2\right]
\end{align}
and with $\phi'^{j}=R^j_{\;k}\phi^k$
\begin{align}
    S'&=-\frac{1}{2}\sum_{j=1}^n\int d^dx\left[\eta^{\alpha\beta}\partial_\alpha R^j_{\;a}\phi^a\partial_\beta R^j_{\;b}\phi^b+m^2(R^j_{\;a}\phi^a\cdot R^j_{\;b}\phi^b)\right]
\end{align}
\textcolor{red}{Not done yet}

\subsubsection*{Problem 1.3.1}
\subsubsection*{Problem 1.3.2}
\subsubsection*{Problem 1.3.3}
\subsubsection*{Problem 1.3.4}
\subsubsection*{Problem 1.3.5}
